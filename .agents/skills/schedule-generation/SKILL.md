...
6. Define `XLSX_PATH` and `XML_PATH` in output folder; use a temporary CSV path only for XML generation
7. In `if __name__ == "__main__":` call `_generate_excel()`, write the temporary CSV, subprocess `generate_msp_xml.py`, then remove the temporary CSV
8. Run script to produce XLSX and XML
...
```python
TEMP_CSV_PATH = HERE / "active-projects" / PROJECT_NAME / f"{PROJECT_NAME}_Project_Schedule.tmp.csv"
...
- [ ] XML file generated and validated, with no retained CSV in the final output folder
